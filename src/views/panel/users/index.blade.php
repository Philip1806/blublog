...
@section('content')
    <div class="row">
        <div class="col-sm-10">
            <a href="{{ route('blublog.users.create') }}"
                class="btn btn-primary btn-block">{{ __('blublog.adduser') }}</a>
        </div>
        <div class="col-sm">
            ...
            <table class="table table-hover">
                <tbody>
                    @foreach ($users as $user)
                        ...
                        <td>{{ $user->email }}</td>
                        <td><a href="{{ route('blublog.users.edit', $user->id) }}"
                                class="btn btn-warning btn-block">{{ __('blublog.edit') }}</a></td>
