...
class BlublogFrontController extends Controller
{
    ...
    public function __construct()
    {
        ...
        if (!Cache::has('blublog.is_banned_' . $ip)) {
            $is_banned = Ban::is_banned($ip);
            Cache::put('blublog.is_banned_' . $ip, $is_banned,  now()->addMinutes(config('blublog.setting_cache')));
        } else {
            $is_banned = Cache::get('blublog.is_banned_' . $ip);
        }
        if ($is_banned) {
            abort(403);
        }
